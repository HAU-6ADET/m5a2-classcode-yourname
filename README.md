...
Your work goes in **`lib/catch_screen.dart`**.

Make `CatchScreen` a `StatefulWidget` with:
...
Concepts to research: `TextEditingController`, `TextField`, `String.trim()`, updating a `List` in `setState`.
...
3. **Name it by the convention** `m<module>a<activity>-<classcode>-<yourname>`.
   For this activity that's **`m5a2-<classcode>-yourname`** (e.g.
   `m5a2-1234-juandelacruz`). The `<classcode>` must match the one in
   `student.json`.
...
```bash
git clone https://github.com/HAU-6ADET/m5a2-<classcode>-yourname.git
...
- ✅ has a TextField and a Catch button, starts empty
- ✅ catching a name adds it
- ✅ catching two names shows both
